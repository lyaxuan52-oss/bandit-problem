...
function save_results(results, save_path)
    ...
    filename = fullfile(save_path, sprintf('epsilon0_results_%s.mat', timestamp));
    ...
    fprintf(fid, '被试编号\tEpsilon0值\t文件名\n');
...
function epsilon0 = epsilon0_estimation_internal(fullpath)
  ...
  feedback = nan(n_rounds, max_trials); 
    ...
    % 定义目标函数
    likelihood = @(eps0) compute_total_likelihood(eps0, choices, feedback);
    
    % 使用模拟退火算法
    [epsilon0, fval] = simulannealbnd(likelihood, 0.5, 0.1, 1, options);
    
    fprintf('最终epsilon0值: %.4f, likelihood: %.4f\n', epsilon0, fval);
end
